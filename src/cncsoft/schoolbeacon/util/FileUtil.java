	...
	public static ArrayList<String> savePicture(File sourceFile,ArrayList<String> lstFileImage, int rotation){
		try {
			...
			try {
		        ...
		        //if (sdkVersion >= 7) {
		        	cncsoft.schoolbeacon.util.CompressImageApi7.Compress(filepath, outStream, bm, rotation);
		        //} else {
		...
		try {
			...
			try {
				if (Constants.IMAGE_COMPRESS) {
					...
					int sdkVersion = Integer.parseInt(Build.VERSION.SDK);
			        if (sdkVersion >= 7) {
			        	cncsoft.schoolbeacon.util.CompressImageApi7.Compress(filepath, outStream, bm);
			        } else {
			        	bm.compress(Bitmap.CompressFormat.JPEG, Constants.JPEG_COMPRESS, outStream);
			        }
