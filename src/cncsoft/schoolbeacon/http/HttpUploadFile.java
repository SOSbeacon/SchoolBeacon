...
import cncsoft.schoolbeacon.util.FileUtil;
                ...
                for (int i = 0; i < listFile.size(); i++) {
                    if (type == TYPE_IMAGE) {
                        filePath = Environment.getExternalStorageDirectory().getAbsolutePath()
                                + "/" + IMAGE_DIR + "/" + listFile.get(i).toString();
                    }
                    if (type == TYPE_VOICE) {
                        filePathinput = Environment.getExternalStorageDirectory().getAbsolutePath()
                                + "/" + VOICE_DIR + "/" + listFile.get(i).toString();
                        //convert to raw ARM NB
                        File fileaudio = new File(filePathinput);
                        ThreegpReader threegpReader = new ThreegpReader(fileaudio);
                        filePath = Environment.getExternalStorageDirectory().getAbsolutePath()
                                + "/" + VOICE_DIR + "/" + AUDIO_FILE_NAME;
                    ...
                    if (recodFile.exists()) {
                        ...
                        try {
                            HttpData httpData = HttpRequest.postRequest(client, uploadUrl,
                            ...
                            logger.log(Level.INFO, "upload file: " + filePath
                                    + " - server response: " + jsonUploadResultString);
                        } catch (Exception e) {
                            logger.log(Level.ERROR,
                                    "Upload error, type: " + TYPE_IMAGE + ", " + e.getMessage());
                            if (type == TYPE_IMAGE) {
                                msg = new Message();
                                msg.what = UPLOAD_IMAGE_FAIL;
                                msg.obj = "Upload image \"" + listFile.get(i).toString()
